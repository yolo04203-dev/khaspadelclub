...
-- Index for fast americano player lookups
CREATE INDEX IF NOT EXISTS idx_americano_players_user_id ON public.americano_players(user_id);

-- Unified stats RPC function
CREATE OR REPLACE FUNCTION public.get_player_unified_stats(
  p_user_id UUID,
...
BEGIN
  ...
  IF v_team_id IS NOT NULL THEN
    SELECT t.name INTO v_team_name FROM teams t WHERE t.id = v_team_id;
  END IF;

  -- Get ladder rank info
  IF v_team_id IS NOT NULL THEN
    SELECT lr.rank, lr.points, lr.streak 
    INTO v_rank, v_points, v_streak
    FROM ladder_rankings lr WHERE lr.team_id = v_team_id LIMIT 1;
  END IF;

  -- LADDER STATS
  IF v_team_id IS NOT NULL THEN
    SELECT 
      COALESCE(SUM(CASE WHEN m.winner_team_id = v_team_id THEN 1 ELSE 0 END), 0),
      COALESCE(SUM(CASE WHEN m.winner_team_id IS NOT NULL AND m.winner_team_id != v_team_id THEN 1 ELSE 0 END), 0)
    INTO v_ladder_wins, v_ladder_losses
    FROM matches m
    WHERE m.status = 'completed'
      AND (m.challenger_team_id = v_team_id OR m.challenged_team_id = v_team_id)
  ...
  IF v_team_id IS NOT NULL THEN
    SELECT
      COALESCE(SUM(CASE WHEN tm2.winner_team_id = v_team_id THEN 1 ELSE 0 END), 0),
      COALESCE(SUM(CASE WHEN tm2.winner_team_id IS NOT NULL AND tm2.winner_team_id != v_team_id THEN 1 ELSE 0 END), 0)
    INTO v_tournament_wins, v_tournament_losses
    FROM tournament_matches tm2
    WHERE tm2.completed_at IS NOT NULL
      AND (tm2.team1_id = v_team_id OR tm2.team2_id = v_team_id)
      AND tm2.completed_at >= v_start_date;
  END IF;

  -- AMERICANO INDIVIDUAL STATS
  SELECT
    COALESCE(SUM(
      CASE 
  ...
  -- RECENT MATCHES (unified, last 15)
  WITH all_matches AS (
    -- Ladder matches
    SELECT 
      m.id,
      ...
      CASE WHEN m.challenger_team_id = v_team_id THEN ct.name ELSE cht.name END AS opponent_name,
      CASE WHEN m.winner_team_id = v_team_id THEN 'win' ELSE 'loss' END AS result,
      CASE WHEN m.challenger_team_id = v_team_id 
    ...
    SELECT
      ...
      CASE WHEN tm2.team1_id = v_team_id THEN t2.name ELSE t1.name END AS opponent_name,
      CASE WHEN tm2.winner_team_id = v_team_id THEN 'win' ELSE 'loss' END AS result,
      CASE WHEN tm2.team1_id = v_team_id
  ...
  WITH daily AS (
    ...
    SELECT m.completed_at::DATE AS match_date,
      CASE WHEN m.winner_team_id = v_team_id THEN 1 ELSE 0 END AS is_win
    FROM matches m
    WHERE v_team_id IS NOT NULL AND m.status = 'completed'
      AND (m.challenger_team_id = v_team_id OR m.challenged_team_id = v_team_id)
      AND m.completed_at >= v_start_date
    ...
    SELECT tm2.completed_at::DATE,
      CASE WHEN tm2.winner_team_id = v_team_id THEN 1 ELSE 0 END
    FROM tournament_matches tm2
    WHERE v_team_id IS NOT NULL AND tm2.completed_at IS NOT NULL
      AND (tm2.team1_id = v_team_id OR tm2.team2_id = v_team_id)
      AND tm2.completed_at >= v_start_date
  ...
  -- HEAD TO HEAD (ladder + tournament only)
  IF v_team_id IS NOT NULL THEN
    WITH h2h AS (
      SELECT
        CASE WHEN m.challenger_team_id = v_team_id THEN m.challenged_team_id ELSE m.challenger_team_id END AS opp_id,
        CASE WHEN m.winner_team_id = v_team_id THEN 1 ELSE 0 END AS is_win
      FROM matches m
      WHERE m.status = 'completed'
        AND (m.challenger_team_id = v_team_id OR m.challenged_team_id = v_team_id)
      ...
      SELECT
        CASE WHEN tm2.team1_id = v_team_id THEN tm2.team2_id ELSE tm2.team1_id END,
        CASE WHEN tm2.winner_team_id = v_team_id THEN 1 ELSE 0 END
      FROM tournament_matches tm2
      WHERE tm2.completed_at IS NOT NULL
        AND (tm2.team1_id = v_team_id OR tm2.team2_id = v_team_id)
